...
thumbnails.forEach(thumbnail => {
    thumbnail.addEventListener('click', event=>{
        ...
        imageContainer.style.backgroundImage = `url('../imagenes/ram-fury-${event.target.id}.jpg')`
...
modalthumbnails.forEach(modalthumbnail => {
    modalthumbnail.addEventListener('click', event=>{
        ...
        modalImageContainer.style.backgroundImage = `url('../imagenes/ram-fury-${event.target.id.slice(-1)}.jpg')`
    });
});
...
function drawProductInModal(){
    ...
    `<div class="cart-modal__details-container">
            <img class="cart-modal__image" src="../imagenes/ram-fury.jpg" alt="">
            <div>
            <p class="cart-modal__product">Kingston FURY Beast 8GB...</p>
            <p class="cart-modal__price">S/225 x3</p>
            </div>
    ...
    deleteProduct()
    let priceModal = document.querySelector('.cart-modal__price');
    priceModal.innerHTML = `S/225 x${lastValue}<span>S/${lastValue*225}.00</span>`;
}
