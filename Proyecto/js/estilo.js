...
thumbnails.forEach(thumbnail => {
    thumbnail.addEventListener('click', event=>{
        ...
        imageContainer.style.backgroundImage = `url('../imagenes/image-product-${event.target.id}.jpg')`
    });
});
...
modalthumbnails.forEach(modalthumbnail => {
    modalthumbnail.addEventListener('click', event=>{
        ...
        modalImageContainer.style.backgroundImage = `url('../imagenes/image-product-${event.target.id.slice(-1)}.jpg')`
    });
});
...
// Mostrar el navbar cuando presiono el menu 
...
function drawProductInModal(){
    ...
    `<div class="cart-modal__details-container">
            <img class="cart-modal__image" src="../imagenes/image-product-1-thumbnail.jpg" alt="">
            <div>
            <p class="cart-modal__product">Hyperx Pulsefire Hast...</p>
            <p class="cart-modal__price">S/125 x3 <span>S/375.00</span> </p>
            </div>
            <img class="cart-modal__delete" src="../imagenes/icon-delete.svg" alt="delete">
        </div>
        <button class="cart-modal__chekount" >Checkout</button>`
    deleteProduct()
    let priceModal = document.querySelector('.cart-modal__price');
    priceModal.innerHTML = `S/125 x${lastValue} <span>S/${lastValue*125}.00</span>`;
}
...
function changeNextImage(imgContainer){
    ...
    imgContainer.style.backgroundImage = `url('../imagenes/image-product-${imgIndex}.jpg')`
}
